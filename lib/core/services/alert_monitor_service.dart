...
class AlertMonitorService with WidgetsBindingObserver {
  static final AlertMonitorService _instance = AlertMonitorService._internal();
  ...
  Future<void> _tick() async {
    if (Platform.isAndroid && !_isAppResumed) return;
  ...
  Future<void> _maybeRunFullSweep() async {
    ...
    await settings.load();
    final interval = Duration(minutes: settings.alertCheckIntervalMinutes);
  ...
  Future<void> _fireBatch(List<NotificationModel> notifications) async {
    final pending = notifications
        .where((notification) => !_visibleAlertIds.contains(notification.id))
        .toList();
    if (pending.isEmpty) return;
    final ids = pending.map((notification) => notification.id).toSet();
    _visibleAlertIds.addAll(ids);
    try {
      unawaited(
        AlertSoundService().playForTypes(
          pending.map((notification) => notification.type),
        ),
      );
      await NotificationsRepository().markFiredMany(ids);
      await showRecurringAlertsDialog(pending);
    } finally {
      _visibleAlertIds.removeAll(ids);
    }
  }
